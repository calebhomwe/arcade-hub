(function () {
  ...
  if (!wrap) return;
  const SUBS = ['The king has left the building.', 'Skill issue detected (theirs).', 'GG EZ (be nice).', 'Absolute cinema.', 'Someone call an ambulance... but not for you.', 'The crown stays in the family.', 'Frostbite? No — CHECKMATE.', 'Board = solved.', 'The board has spoken.', 'Somewhere, a chess coach is smiling.', 'That one goes in the scrapbook.', 'Clean. Ruthless. Framed on the wall.', 'The pieces never stood a chance.', 'Checkmate, with style points.', 'A masterpiece in under forty moves.', 'File that under: unforgettable.'];
